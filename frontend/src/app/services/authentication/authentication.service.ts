...
import type { TokenResponse, UserProfile } from '../../model/download.models';
...
export class AuthService {
  ...
  constructor() {
    this.sessionReady = new Promise(r => { this._resolveSession = r; });
  ...
  private tryRestoreSession(): void {
    ...
    this.http.get<UserProfile>('/api/user/me').subscribe({
      next:  u  => { this.currentUser.set(u); this._resolveSession(); },
      error: () => { this._resolveSession(); }
    });
  }
