...
@Component({
  selector: 'wssd-landing-instagram',
  standalone: true,
...
<div class="page">
  ...
  <section class="hero">
    <h1>Descargar Videos de Instagram Gratis</h1>
    <p>Pega el enlace y descarga en HD. Sin registro, sin instalar nada.</p>
    <div class="search neo">
      <input type="url" [(ngModel)]="url" placeholder="Pega el enlace de Instagram aquí" class="neo-inset" (keydown.enter)="go()" />
      <button (click)="go()" class="btn">Descargar</button>
    </div>
  </section>
  <section class="features">
    <h2>¿Por qué WSSD para Instagram?</h2>
    <div class="cards">
...
export class LandingInstagramComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.title.setTitle('Descargar Videos de Instagram Gratis 2026 | WSSD');
    this.meta.updateTag({ name: 'description', content: 'Descarga videos de Instagram gratis en HD. Sin instalar nada. WSSD — WS Social Downloader.' });
  }
